...
def lambda_handler(event, context):
    ...
    if 'invokingEvent' in event and 'configurationItem' in json.loads(event['invokingEvent']):
...
def remediate_resource(resource_type, resource_id):
    ...
    if result['status'] == 'SUCCESS':
        ...
        log_remediation(resource_type, resource_id, original_state, remediated_state, result)
...
def remediate_ebs_volume(volume_id):
    ...
    try:
        ...
        logger.info(f"Attaching new encrypted volume {new_volume_id} to instance {instance_id}")
...
def remediate_s3_bucket(bucket_name):
    ...
    try:
        ...
        return {
            'status': 'FAILED',
            'message': f"Error remediating S3 bucket: {str(e)}"
        }
    ...
    try:
        ...
        if resource_type == 'AWS::EC2::Volume':
            ...
            try:
                ...
                bucket_details['Encryption'] = encryption.get('ServerSideEncryptionConfiguration', {})
            except ClientError:
        ...
        logger.error(f"Error capturing original state for {resource_type} {resource_id}: {str(e)}")
        return {}

def log_remediation(resource_type, resource_id, original_state, remediated_state, result):
    """
    ...
    if EVIDENCE_BUCKET:
        try:
            ...
            logger.info(f"Remediation evidence saved to s3://{EVIDENCE_BUCKET}/{evidence_key}")
        except Exception as e:
