            ...
            if 'Statement' in policy_json:
                ...
                for statement in original_statements:
                    ...
                    if is_public_statement(statement):
                        logger.info(f"Removing public statement from bucket policy: {json.dumps(statement)}")
                        modified = True
                    else:
                ...
                if modified:
                    if filtered_statements:
                        ...
                        logger.info(f"Updating bucket policy with {len(filtered_statements)} non-public statements")
                        ...
                        logger.info(f"Deleting bucket policy as all statements were public")
...
def capture_current_state(bucket_name):
    ...
    try:
        ...
        state['public_access_block'] = public_access_block['PublicAccessBlockConfiguration']
    except Exception as e:
