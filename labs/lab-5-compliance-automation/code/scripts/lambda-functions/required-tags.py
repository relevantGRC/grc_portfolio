...
def evaluate_compliance(configuration_item, required_tags, required_tag_values):
    ...
    if 'tags' not in configuration_item['configuration'] or not configuration_item['configuration']['tags']:
    ...
    for tag_key, expected_value in required_tag_values.items():
        if tag_key in resource_tags:
            if expected_value != '' and resource_tags[tag_key] != expected_value:
                non_compliant_values.append(f"{tag_key} should be '{expected_value}' but is '{resource_tags[tag_key]}'")
    
    if non_compliant_values:
        return 'NON_COMPLIANT', f"Tag values don't match requirements: {', '.join(non_compliant_values)}"
...
def get_configuration_item(invoking_event):
    ...
    try:
        ...
        raise Exception(f"Could not get configuration item from invoking event: {str(e)}")
