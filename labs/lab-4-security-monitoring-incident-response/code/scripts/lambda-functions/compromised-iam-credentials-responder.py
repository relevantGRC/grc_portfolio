...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'detail' in event and 'type' in event['detail'] and 'GuardDuty' in event['detail']['type']:
            # This is a GuardDuty finding
            return handle_guardduty_finding(event)
        elif 'detail' in event and 'eventSource' in event['detail'] and event['detail']['eventSource'] == 'iam.amazonaws.com':
...
def handle_guardduty_finding(event):
    ...
    try:
        ...
        if not principal_id:
            logger.warning(f"Could not determine affected IAM principal from finding {finding_id}")
            return {
                'statusCode': 400,
                'body': json.dumps({'error': 'Could not determine affected IAM principal'})
            }
        ...
        if severity >= 7.0:  # High severity
            if principal_type == 'user' and access_key_id:
                ...
                containment_actions.append(f"Disabled access key {access_key_id} for user {principal_id}")
            
            # Apply a restrictive policy to the principal
            apply_restrictive_policy(principal_type, principal_id)
            containment_actions.append(f"Applied restrictive policy to {principal_type} {principal_id}")
...
def handle_cloudtrail_event(event):
    ...
    try:
        ...
        if principal_type == 'iamuser':
            ...
            principal_id = user_identity.get('sessionContext', {}).get('sessionIssuer', {}).get('userName')
        
        if not principal_id:
            logger.warning(f"Could not determine affected IAM principal from CloudTrail event {event_name}")
            return {
                'statusCode': 400,
                'body': json.dumps({'error': 'Could not determine affected IAM principal'})
            }
            ...
            if event_name == 'CreateAccessKey':
                ...
                if 'accessKey' in response_elements:
                    ...
                    if access_key_id:
                        ...
                        containment_actions.append(f"Disabled newly created access key {access_key_id} for user {principal_id}")
...
def collect_evidence(principal_type, principal_id):
    ...
    try:
        ...
        if principal_type == 'user':
            ...
            evidence['configuration']['access_keys'] = access_keys.get('AccessKeyMetadata', [])
            
            # Get attached policies
            attached_policies = iam_client.list_attached_user_policies(UserName=principal_id)
            evidence['configuration']['attached_policies'] = attached_policies.get('AttachedPolicies', [])
        elif principal_type == 'role':
            role = iam_client.get_role(RoleName=principal_id)
            evidence['configuration']['role'] = role.get('Role', {})
            
            # Get attached policies
            attached_policies = iam_client.list_attached_role_policies(RoleName=principal_id)
            evidence['configuration']['attached_policies'] = attached_policies.get('AttachedPolicies', [])
    except Exception as e:
...
def apply_restrictive_policy(principal_type, principal_id):
    ...
    try:
        ...
        policy_name = f"SecurityContainment-{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
        policy_document = {
            "Version": "2012-10-17",
            "Statement": [
                {
        ...
        logger.info(f"Applied restrictive policy {policy_name} to {principal_type} {principal_id}")
    except Exception as e:
    ...
    try:
        ...
        response = ssm_client.start_automation_execution(
            ...
            Parameters={
                ...
                'IncidentTimestamp': [incident_report.get('timestamp', datetime.datetime.now().isoformat())],
                'EvidenceBucket': [EVIDENCE_BUCKET] if EVIDENCE_BUCKET else ['none']
            }
        )
        
        logger.info(f"Triggered incident workflow {INCIDENT_WORKFLOW_DOCUMENT}, execution ID: {response.get('AutomationExecutionId')}")
    except Exception as e:
