...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not encryption_status['is_encrypted'] or (KMS_KEY_ID and encryption_status['kms_key_id'] != KMS_KEY_ID):
            # Object is not encrypted or not encrypted with the correct key
            logger.info(f"Object {object_key} in bucket {bucket_name} is not properly encrypted. Remediating...")
            ...
            logger.info(f"Object {object_key} in bucket {bucket_name} is already properly encrypted")
            return {
                'statusCode': 200,
                'body': json.dumps({
...
def check_encryption(bucket_name, object_key):
    ...
    try:
        ...
        logger.error(f"Error checking encryption for {object_key} in bucket {bucket_name}: {str(e)}")
        raise
...
def remediate_object(bucket_name, object_key):
    ...
    try:
        ...
        logger.info(f"Successfully remediated encryption for {object_key} in bucket {bucket_name}")
    except ClientError as e:
        logger.error(f"Error remediating encryption for {object_key} in bucket {bucket_name}: {str(e)}")
        raise
